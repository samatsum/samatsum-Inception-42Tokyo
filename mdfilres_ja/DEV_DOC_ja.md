...
* **メモリ (RAM)**: 最小4GB、**8GB推奨**（8.4GBでテスト済み）。メモリ内キャッシュやMariaDBを安定して動作させるために不可欠です。
* **ストレージ**: 少なくとも15〜20GBの空きディスク容量。
...
パスワードは `.env` に保存しては**いけません**。`secrets/` ディレクトリ（Gitの管理対象外）に保存してください。これらは実行時にコンテナ内へ安全にマウントされます。
